...
function removeUrlSearchParameters(urlString) {
...
function listenToVideoElementLoadedIn() {
        ...
        for(let record of mutationRecords) {
            if (record.type === 'childList') {
                record.addedNodes.forEach(node => {
                    if (node.id === 'video-card-normal') {
                        observer.disconnect();
                        observerConfig.subtree = true;
                        observer.observe(node, observerConfig);
                    }
                    if (node.className && node.className.indexOf('FinishCover') > -1) {
    ...
    const appDivToListen = document.querySelector('#app .layout-box div[dir="ltr"]');
    if (appDivToListen) {
